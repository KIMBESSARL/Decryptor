...
/* External objects */
extern de_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern de_int line; /* source code line number - defined in scanner.c */
...
extern de_void startParser(de_void);
extern de_int startScanner(BufferPointer sc_buf);
extern de_void printBNFData(ParserData psData);

static de_void displayParser(BufferPointer ptrBuffer);
static de_long getParserFileSize(de_char* fname);
static de_void callGarbageCollector(de_void);
...
de_int main4Parser(de_int argc, de_strg* argv) {

	de_int numParserErrors = 0;			/* Initializes the errors */
	...
	sourceBuffer = readerCreate(READER_DEFAULT_SIZE, READER_DEFAULT_FACTOR, MODE_MULTI);
	...
	printf("DEBUG: loadsize = %d\n", loadsize);
	printf("DEBUG: buffer wrte = %d size = %d\n",
	readerGetPosWrte(sourceBuffer), readerGetSize(sourceBuffer));
	...
	stringLiteralTable = readerCreate(READER_DEFAULT_SIZE, READER_DEFAULT_FACTOR, MODE_MULTI);
...
de_long getParserFileSize(de_char* fname) {
	FILE* input;
	de_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
de_void displayParser(BufferPointer ptrBuffer) {
...
de_void callGarbageCollector(de_void) {
